    ...
    fluidRow(
      sidebarPanel(width = 3,
                   ...
                   conditionalPanel(
                     ...
                     ns = ns,
                     selectInput(ns("xValueTypeC"),
                                 label = "Choose x value limits of comparison",
                                 choices = c("For x greater than" = "gx",
                                             "For x lower than" = "lx",
                     ...
                     conditionalPanel(
                       ...
                       ns = ns,
                       numericInput(ns("compX1C"), label = NULL,
                                    value = NULL, width = "70%")
                     ),
                     conditionalPanel(
                       condition = "input.xValueTypeC == 'intx' ",
                       ns = ns,
                       numericInput(ns("compX2C"), label = "Upper x value",
                                    value = NULL, width = "70%")
                     ),
                   ),
                   conditionalPanel(
                   ...
                   selectInput(ns("xValueType"),
                               label = "Choose x value limits to constrain",
                               choices = c("For all x" = "allx",
                   ...
                   conditionalPanel(
                     ...
                     ns = ns,
                     numericInput(ns("compX1"), label = NULL,
                                  value = NULL, width = "70%")
                   ),
                   conditionalPanel(
                     condition = "input.xValueType == 'intx' ",
                     ns = ns,
                     numericInput(ns("compX2"), label = "Upper x value",
                                  value = NULL, width = "70%")
                   ),
                   tags$hr(),
                   numericInput(ns("unc"), label = "Uncertainty", min = 0,
                                value = 0, width = "70%")),
                   tags$hr(),
                   textInput(ns("plotName"), NULL, value = "",
...
postProcessing <- function(input, output, session, savedData) {
  ...
  observeEvent(input$postPlot, {
    ...
    ppValuesNew <- list(compare = input$compare,
                        compY = input$compY,
    ...
    postPlotStyle$xRange <- getRange(
      data = postPlotValues$selectedData[, unlist(getSelection(
        postPlotValues$dataSettings$xColumns
        )$colNames), drop = FALSE],
      type = getSelection(
        postPlotValues$dataSettings$xColumns
        )$type,
      credPercent = getSelection(
        postPlotValues$dataSettings$xColumns
        )$credPercent,
    ...
    postPlotStyle$yRange <- getRange(
      data = postPlotValues$selectedData[, unlist(getSelection(
        postPlotValues$dataSettings$yColumns
        )$colNames), drop = FALSE],
      type = getSelection(
        postPlotValues$dataSettings$yColumns
        )$type,
